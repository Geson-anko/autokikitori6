...
class VoiceBand(pl.LightningModule):
    ...
    def __init__(self,hparams:hparams):
        ...
        # set griffin lim, mel scaler and spectrogram
    ...
    def forward(self,target:torch.Tensor):
        ...
        spect = (voice.expm1() * gain)
        spect = torch.clamp(spect,max=10000)
        mel = self.mel_scaler(spect).log1p()
        return mel
    ...
    def training_step(self,batch,idx):
        target, = batch
        self.target = target
        target = self.mel_scaler(target.expm1()).log1p()
        out = self(target)
